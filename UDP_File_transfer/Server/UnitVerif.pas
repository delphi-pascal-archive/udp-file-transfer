...
// Renvoi false si les fichiers sont corrompu (Fonction principale)
function Verifier(const S : String): boolean;
...
function Verifier(const S : String): boolean;
var ListFichier,ListFichierReel: TMD5Data;
begin
  Result:=false;
  ListFichier:=MD5StrToMD5Data(s);
  ListFichierReel:=MD5DataFromFile(FileNameList);
  if not(MD5MemEqual(ListFichier,ListFichierReel)) then
  begin
    exit;
  end;
end;
